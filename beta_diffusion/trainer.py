...
from run_car_racing import Tester
from cart_racing_v2 import CarRacing
from models import Model_Cond_Diffusion, Model_cnn_mlp, Model_cnn_bc, Model_Cond_Beta_Diffusion
...
class Trainer():
    def __init__(self, n_epoch, lrate, device, n_hidden, batch_size, n_T,
                 ...
                 losstype, eta, scale, shift, sigmoid_power, sigmoid_start,
                 sigmoid_end, name='', param_search=False,
                 embedding="Model_cnn_mlp", dataset_origin="human"):
    ...
    def main(self):
        if self.run_wandb:
            self.config_wandb(project_name="OpenAI-Car-Racing-Article-Beta-Diffuser", name=self.name)
    ...
    def create_agent_model(self, conv_model, x_dim, y_dim):
        return Model_Cond_Beta_Diffusion(
            conv_model,
            betas=self.betas,
            n_T = self.n_T,
            device=self.device,
            ...
            scale=self.scale,
            shift=self.shift,
            sigmoid_power=self.sigmoid_power,
            sigmoid_start=self.sigmoid_start,
            sigmoid_end=self.sigmoid_end
        ).to(self.device)
    ...
    def save_model(self, model, name, ep=''):
        ...
        torch.save(self.model.state_dict(), os.getcwd()+'/model_pytorch/beta_diff/'+self.dataset_path.split(os.sep)[1]+'/'+self.dataset_path.split(os.sep)[2]+'_'+self.get_git_commit_hash()+'_ep_'+f'{ep}'+'.pkl')
