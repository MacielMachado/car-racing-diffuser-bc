...
class CarRacingCustomDataset(Dataset):
    def __init__(
        ...
        # dim = (32, 32)
        # resized = np.array([cv2.resize(self.image_all[i], dim, interpolation=cv2.INTER_AREA) for i in range(len(self.image_all))])
        # self.image_all = resized        
...
def plot_observation(img_array):
    # Plot the environment observation
    fig, ax = plt.subplots(1,1)
    ax.imshow(img_array, cmap=plt.get_cmap("gray"))
